...
// Cada passo aparece numa posição diferente da tela
const STEPS = [
  {
    id: 1,
    emoji: '👋',
    ...
    badge: 'Passo 1: Menu lateral → Dashboard (você está aqui)',
  ...
  },
  {
    ...
    badge: 'Passo 2: Integrações → Webhook',
  ...
  },
  {
    ...
    badge: 'Passo 3: Integrações → WhatsApp',
    position: { bottom: '80px', left: '24px' },
  ...
  },
  {
    ...
    badge: 'Passo 4: Integrações → Meta Ads',
    position: { top: '80px', left: '24px' },
    primaryLabel: 'Configurar agora',
    secondaryLabel: 'Concluir',
    primaryNav: '/meta-ads',
  },
];
...
export function TutorialPopup() {
  ...
  return (
    <>
      {/* Backdrop suave — não bloqueia a tela toda, só escurece levemente */}
      <div
        style={{
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
                {STEPS.map(s => (
                  <div
                    key={s.id}
                    style={{
                      width: s.id === step ? '16px' : '6px',
                      height: '4px',
                      borderRadius: '99px',
                      background: s.id === step ? '#16a34a' : (dark ? '#27272a' : '#e5e7eb'),
          ...
          <button
            ...
            style={{
              ...
              border: 'none', background: dark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)',
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '4px 10px', borderRadius: '99px', background: dark ? 'rgba(22,163,74,0.12)' : '#f0fdf4', border: '1px solid rgba(22,163,74,0.2)', fontSize: '11.5px', color: '#16a34a', fontWeight: 500, marginBottom: '10px' }}>
          📍 {current.badge}
        </div>

        {/* Texto */}
        <p style={{ fontSize: '13px', color: txtMid, lineHeight: 1.6, margin: '0 0 16px' }}>
          {current.text}
        </p>
        ...
        {step === 2 && (
          <div style={{
            ...
            <span style={{
              flex: 1, fontSize: '10.5px', color: txtMid,
        ...
        <div style={{ display: 'flex', gap: '8px', flexDirection: 'column' }}>
          ...
          {current.secondaryLabel && (
            <button
              onClick={() => {
                if (current.secondaryLabel === 'Concluir') { close(); return; }
                next();
              }}
              style={{
                flex: 1, padding: '9px', borderRadius: '9px',
                border: `1px solid ${border}`, background: 'transparent',
                color: txtMid, fontSize: '13px', fontWeight: 500,
          ...
          <button
            onClick={() => {
              if (current.primaryNav) {
                close();
                navigate(current.primaryNav);
                return;
              }
              next();
            }}
          ...
          >
            ...
            {!current.primaryNav && step < TOTAL_STEPS && (
