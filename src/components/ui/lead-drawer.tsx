...
import { MotivoModal } from './motivo-modal';
...
function cleanCampaignName(raw?: string | null): string | null { if (!raw) return null; return raw.replace(/\|\d+$/, '').trim(); }
    ...
    <div style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
      <button onClick={() => setActiveKey(open ? null : openKey)} style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 0', background:'none', border:'none', cursor:'pointer', fontFamily:FONT, WebkitTapHighlightColor:'transparent' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
          ...
          <span style={{ fontSize:'13.5px', fontWeight:500, color:dark?'#f4f4f5':'#1f2937', letterSpacing:'-0.01em', fontFamily:FONT }}>{title}</span>
        </div>
        <ChevronDown style={{ width:'14px', height:'14px', color:'#d1d5db', flexShrink:0, transform:open?'rotate(180deg)':'rotate(0deg)', transition:'transform 0.25s cubic-bezier(0.4,0,0.2,1)', willChange:'transform' }} />
      </button>
      <div style={{ height:`${height}px`, overflow:'hidden', transition:'height 0.28s cubic-bezier(0.4,0,0.2,1)', willChange:'height' }}>
        <div ref={contentRef} style={{ paddingBottom:'12px', display:'flex', flexDirection:'column', gap:'10px', willChange:'transform' }}>{children}</div>
      </div>
    </div>
  );
}
...
function Field({ label, value, dark }: { label: string; value?: string | null; dark: boolean }) {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize:'13.5px', color:dark?'#d4d4d8':'#374151', lineHeight:1.5, fontFamily:FONT }}>{value}</p>
    </div>
  );
}
...
function DeleteConfirm({ name, onConfirm, onCancel, loading, dark }: { name: string; onConfirm: ()=>void; onCancel: ()=>void; loading: boolean; dark: boolean; }) {
  return (
    <>
      ...
      <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', zIndex:63, background:dark?'#111113':'#fff', borderRadius:'16px', padding:'24px', width:'88%', maxWidth:'340px', boxShadow:dark?'0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.05)':'0 20px 60px rgba(0,0,0,0.15)', fontFamily:FONT, animation:'ld-up 0.2s cubic-bezier(0.32,0.72,0,1)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'12px' }}>
          <div style={{ width:'36px', height:'36px', borderRadius:'10px', background:dark?'rgba(220,38,38,0.1)':'#fff1f2', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
...
export function LeadDrawer({ lead, isOpen, onClose, onUpdate }: LeadDrawerProps) {
  ...
  const dias = lead ? diasSemContato(lead) : 0;
  const showAlerta = lead && status === 2 && dias >= 3;
  ...
  async function applyStatus(newStatus: number, motivo?: string) {
    ...
    if (error) {
      ...
      onUpdate({ ...lead, status: newStatus, motivo_reprovacao: motivo || (lead as any).motivo_reprovacao });
      const label = STATUS.find(s => s.id === newStatus)?.label || 'Atualizado';
      toast.success(label);
    }
  }
  ...
  function handleStatus(i: number) {
    if (!lead) return;
    if (status === i && i !== 4) return;
    if (i === 4) {
      // Reprovado → abre modal de motivo
      setPendingStatus(4);
      setShowMotivo(true);
    } else {
      applyStatus(i);
  ...
  async function handleMotivoConfirm(motivo: string) {
    ...
    if (pendingStatus !== null) {
      await applyStatus(pendingStatus, motivo);
      setPendingStatus(null);
    }
  }
  ...
  const motivoAtual = l.motivo_reprovacao;
  ...
  return (
    <>
      ...
      {showMotivo && <MotivoModal onConfirm={handleMotivoConfirm} onCancel={() => { setShowMotivo(false); setPendingStatus(null); }} dark={dark} />}
      ...
      <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%, -50%)', width:'92%', maxWidth:'480px', maxHeight:'90vh', zIndex:51, fontFamily:FONT, animation:'ld-up 0.24s cubic-bezier(0.32, 0.72, 0, 1)', borderRadius:'22px', background:dark?'rgba(18,18,20,0.96)':'rgba(255,255,255,0.94)', backdropFilter:'blur(28px)', WebkitBackdropFilter:'blur(28px)', boxShadow:dark?'0 24px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.05)':'0 24px 80px rgba(0,0,0,0.13), 0 0 0 1px rgba(255,255,255,0.7)', overflow:'hidden', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ padding:'22px 22px 16px', position:'relative', flexShrink:0 }}>
          <button onClick={onClose} style={{ position:'absolute', top:'16px', right:'16px', width:'26px', height:'26px', background:dark?'rgba(255,255,255,0.05)':'rgba(0,0,0,0.06)', border:'none', borderRadius:'50%', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'background 0.15s' }} onMouseEnter={e=>(e.currentTarget.style.background=dark?'rgba(255,255,255,0.1)':'rgba(0,0,0,0.1)')} onMouseLeave={e=>(e.currentTarget.style.background=dark?'rgba(255,255,255,0.05)':'rgba(0,0,0,0.06)')}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'14px', marginRight:'36px' }}>
            <div style={{ width:'52px', height:'52px', borderRadius:'14px', flexShrink:0, background:`linear-gradient(135deg, ${g1}, ${g2})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'18px', fontWeight:700, color:'#fff', letterSpacing:'-0.02em', boxShadow:`0 4px 14px ${g1}60`, fontFamily:FONT }}>
              {initials(lead.nome)}
            </div>
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:'8px', marginTop:'5px' }}>
                ...
                {lead.instagram && <span style={{ display:'flex', alignItems:'center', gap:'3px', fontSize:'12px', color:dark?'#71717a':'#6b7280', fontFamily:FONT }}><Instagram style={{ width:'11px', height:'11px', strokeWidth:1.8 }}/>{lead.instagram}</span>}
                <span style={{ display:'flex', alignItems:'center', gap:'3px', fontSize:'12px', color:dark?'#52525b':'#b0b7c3', fontFamily:FONT }} title="Última movimentação">
                  <Clock style={{ width:'11px', height:'11px', strokeWidth:1.8 }}/>
                  {(lead as any).ultimo_status_change ? getRelativeTime((lead as any).ultimo_status_change) : getRelativeTime(lead.created_at)}
                </span>
              </div>
            </div>
          </div>
          {showAlerta && (
            <div style={{ marginTop:'14px', padding:'8px 12px', borderRadius:'10px', background:'#ef4444', display:'flex', alignItems:'center', gap:'8px', boxShadow:'0 4px 12px rgba(239,68,68,0.3)', animation:'ld-pulse-red 2s infinite' }}>
              <span style={{ fontSize:'16px' }}>⚠️</span>
              <span style={{ fontSize:'13px', fontWeight:700, color:'#fff' }}>Atenção: {dias} dias sem nenhuma movimentação!</span>
            </div>
          )}
        </div>
        ...
        <div style={{ padding:'14px 22px', flexShrink:0 }}>
          ...
          <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
            {STATUS.map(s => {
              ...
              return (
                <button key={s.id} onClick={() => handleStatus(s.id)} style={{ padding:'5px 11px', borderRadius:'8px', flexShrink:1, border:`1px solid ${active?s.border:dark?'#1e1e22':'#e5e7eb'}`, background:active?(dark?s.darkBg:s.bg):'rgba(0,0,0,0.02)', color:active?(dark?s.darkText:s.text):(dark?'#71717a':'#6b7280'), fontSize:'12px', fontWeight:active?600:400, cursor:'pointer', display:'flex', alignItems:'center', gap:'5px', transition:'all 0.18s cubic-bezier(0.4,0,0.2,1)', fontFamily:FONT, whiteSpace:'nowrap' }}>
                  <span style={{ width:'6px', height:'6px', borderRadius:'50%', background:s.color, flexShrink:0, transition:'transform 0.18s', transform:active?'scale(1.2)':'scale(1)' }}/>
          ...
          {/* Motivo reprovação — aparece se status for Reprovado e tiver motivo salvo */}
          {status === 4 && motivoAtual && (
            <div style={{ marginTop:'10px', padding:'8px 12px', borderRadius:'8px', background:dark?'rgba(239,68,68,0.08)':'#fff1f2', border:`1px solid ${dark?'rgba(239,68,68,0.2)':'#fecaca'}`, display:'flex', alignItems:'center', gap:'6px' }}>
              <span style={{ fontSize:'11px', color:'#ef4444', fontWeight:500, fontFamily:FONT }}>Motivo:</span>
              <span style={{ fontSize:'12px', color:dark?'#f87171':'#dc2626', fontFamily:FONT }}>{motivoAtual}</span>
            </div>
          )}
        </div>
        ...
        <div style={{ overflowY:'auto', flex:1, WebkitOverflowScrolling:'touch' }}>
          <div style={{ padding:'4px 22px 8px' }}>
            <p style={{ fontSize:'10.5px', fontWeight:500, color:dark?'#52525b':'#9ca3af', textTransform:'uppercase', letterSpacing:'0.07em', padding:'8px 0 4px', fontFamily:FONT }}>Informações do Lead</p>

            <Section openKey="goals" activeKey={activeSection} setActiveKey={setActiveSection} dark={dark} icon={<Target style={{ width:'14px', height:'14px', strokeWidth:1.8 }}/>} title="Objetivos">
              <Field label="O que mais te atrai" value={lead.o_que_mais_te_atrai} dark={dark}/>
              <Field label="Quanto quer ganhar" value={lead.quanto_ganha} dark={dark}/>
              <Field label="O que quer conquistar" value={lead.o_que_conquistar} dark={dark}/>
              <Field label="Onde se imagina em 6 meses" value={lead.imagina_6_meses} dark={dark}/>
            </Section>

            <Section openKey="profile" activeKey={activeSection} setActiveKey={setActiveSection} dark={dark} icon={<Home style={{ width:'14px', height:'14px', strokeWidth:1.8 }}/>} title="Perfil Pessoal">
              <Field label="Idade" value={lead.idade} dark={dark}/>
              <Field label="Tem filhos" value={lead.tem_filhos} dark={dark}/>
              <Field label="Mora com alguém" value={lead.mora_com} dark={dark}/>
              <Field label="Situação atual" value={lead.situacao_atual} dark={dark}/>
              <Field label="Rede de apoio" value={lead.rede_apoio} dark={dark}/>
            </Section>

            <Section openKey="exp" activeKey={activeSection} setActiveKey={setActiveSection} dark={dark} icon={<Briefcase style={{ width:'14px', height:'14px', strokeWidth:1.8 }}/>} title="Experiência">
              <Field label="Experiência em vendas" value={lead.experiencia_vendas} dark={dark}/>
              <Field label="Já tentou vender semijoia" value={lead.tentou_semijoia} dark={dark}/>
              <Field label="Consignado" value={lead.consignado} dark={dark}/>
              <Field label="Nome negativado" value={lead.negativado} dark={dark}/>
              <Field label="Aceita regras" value={lead.aceita_regras} dark={dark}/>
            </Section>

            {hasTraffic && (
              <Section openKey="traffic" activeKey={activeSection} setActiveKey={setActiveSection} dark={dark} icon={<Megaphone style={{ width:'14px', height:'14px', strokeWidth:1.8 }}/>} title="Origem do Tráfego">
                ...
                <Field label="Posicionamento" value={l.utm_term} dark={dark}/>
          ...
          <div style={{ padding:'0 22px 20px' }}>
            ...
            <textarea value={obs} onChange={e=>{setObs(e.target.value);setObsChanged(true);}} placeholder="Anotações sobre este lead..." rows={3} style={{ width:'100%', padding:'10px 12px', fontSize:'13.5px', lineHeight:1.55, fontFamily:FONT, color:dark?'#f4f4f5':'#374151', background:dark?'rgba(255,255,255,0.02)':'rgba(0,0,0,0.025)', border:`1px solid ${dark?'rgba(255,255,255,0.08)':'rgba(0,0,0,0.08)'}`, borderRadius:'10px', resize:'none', outline:'none', transition:'border-color 0.18s', boxSizing:'border-box' as any }} onFocus={e=>(e.target.style.borderColor='rgba(59,130,246,0.45)')} onBlur={e=>(e.target.style.borderColor='rgba(0,0,0,0.08)')}/>
          </div>
        </div>
      ...
      <style>{`
        ...
        @keyframes ld-pulse-red { 0%{transform:scale(1)} 50%{transform:scale(1.02)} 100%{transform:scale(1)} }
      `}</style>
    </>
  );
}
