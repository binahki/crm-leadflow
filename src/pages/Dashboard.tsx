...
function buildChartData(leads:Lead[],period:string,from?:string,to?:string){
  ...
  if(days===1){
    ...
    leads.forEach(l=>{const d=parseLeadDate(l.created_at);if(d>=dayStart&&d<=dayEnd){const slotH=Math.floor(d.getHours()/2)*2;const key=`${String(slotH).padStart(2,'0')}h`;if(key in slots)slots[key]++;}});
...
async function fetchMetaData(period:string,from?:string,to?:string):Promise<{metrics:MetaMetrics;campaigns:Campaign[]}>{
  ...
  try{
    ...
    let timeParam=period in presetMap?`date_preset=${presetMap[period]}`:period==='custom'&&from&&to?`time_range=%7B%22since%22%3A%22${from}%22%2C%22until%22%3A%22${to}%22%7D`:'date_preset=this_month';
...
export default function Dashboard() {
  ...
  const loadMeta=async()=>{
    ...
    try{const{metrics,campaigns}=await fetchMetaData(selectedPeriod,customFrom,customTo);setMetaMetrics(metrics);setMetaCampaigns(campaigns);if(metrics.spend===0&&campaigns.length===0)setMetaError(true);}
  ...
  const periodLabel=selectedPeriod==='custom'&&customFrom&&customTo?`${isoToBR(customFrom)} – ${isoToBR(customTo)}`:PERIOD_FILTERS.find(p=>p.value===selectedPeriod)?.label??'Este mês';
    ...
    <AppLayout leadCount={allLeads.length}>
      <div style={{padding:pad,background:bg,minHeight:'100vh'}}>
        ...
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'20px',flexWrap:'wrap',gap:'10px'}}>
          <div>
            <h1 style={{fontSize:isMobile?'20px':'26px',fontWeight:700,color:txtHi,letterSpacing:'-0.03em',margin:0}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr 1fr':'repeat(4,1fr)',gap:isMobile?'10px':'16px',marginBottom:'16px'}}>
          {[
            ...
            <div key={i} style={{background:cardBg,borderRadius:'14px',padding:isMobile?'14px':'20px',border:`1px solid ${border}`}}>
              ...
              <p style={{fontSize:isMobile?'20px':'26px',fontWeight:700,color:txtHi,letterSpacing:'-0.03em',margin:'0 0 6px'}}>{c.value}</p>
        ...
        <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:'14px'}}>
          ...
          <div style={{background:cardBg,borderRadius:'14px',padding:isMobile?'16px':'24px',border:`1px solid ${border}`}}>
            ...
            {metaLoading?[...Array(3)].map((_,i)=><div key={i} style={{height:'32px',borderRadius:'8px',background:dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.04)',marginBottom:'8px'}}/>)
            :metaError||campRows.length===0?<div style={{textAlign:'center',padding:'20px 0'}}><p style={{fontSize:'13px',color:txtMid,margin:0}}>{metaError?'Erro ao conectar ao Meta Ads':'Nenhuma campanha'}</p></div>
            :(
              <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                {campRows.map((row,i)=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px 0',borderBottom:i<campRows.length-1?`1px solid ${divCls}`:'none'}}>
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{fontSize:'12.5px',fontWeight:500,color:txtHi,margin:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{row.name}</p>
                      <p style={{fontSize:'11px',color:txtMid,margin:'2px 0 0'}}>{row.spend} · {row.leads} leads · {row.cpl}</p>
                    </div>
                    <div style={{display:'flex',alignItems:'center',gap:'5px',flexShrink:0}}>
                      <div style={{height:'4px',width:'40px',borderRadius:'99px',background:dark?'rgba(255,255,255,0.07)':'rgba(0,0,0,0.07)',overflow:'hidden'}}>
