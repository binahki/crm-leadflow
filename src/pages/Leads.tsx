...
function parseLeadDate(str?: string | null): Date {
  ...
  if (match) { const [, d, m, y, h = '0', min = '0'] = match; return new Date(Number(y), Number(m) - 1, Number(d), Number(h), Number(min)); }
  return new Date(str);
}
...
function filterByPeriod(leads: Lead[], period: string, customFrom?: string, customTo?: string): Lead[] {
  ...
  const now = new Date(); const ts = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0, 0); const te = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59, 999);
  const inR = (l: Lead, a: Date, b: Date) => { const d = parseLeadDate(l.created_at); return d >= a && d <= b; };
  switch (period) {
...
export default function LeadsPage() {
  ...
  // Lê parâmetro ?campanha= da URL (redirect da página de Campanhas)
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    if (campanhaFilter.trim()) { const q = campanhaFilter.toLowerCase(); r = r.filter(l => { const lany = l as any; return (lany.utm_campaign || '').toLowerCase().includes(q) || (lany.utm_source || '').toLowerCase().includes(q); }); }
    return r;
      ...
      <div style={{ padding: isMobile ? '12px' : '28px', background: bg, minHeight: '100vh' }}>
              ...
              <>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    style={{ paddingLeft: '10px', paddingRight: campanhaFilter ? '28px' : '10px', paddingTop: '7px', paddingBottom: '7px', borderRadius: '9px', border: `1px solid ${campanhaFilter ? '#2563eb' : border}`, background: dark ? '#111113' : '#fff', color: dark ? '#d4d4d8' : '#374151', fontSize: '13px', outline: 'none', width: '180px', fontFamily: 'inherit' }}
                  />
                  {campanhaFilter && (
                    <button onClick={() => setCampanhaFilter('')} style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: dark ? '#71717a' : '#9ca3af', display: 'flex', padding: 0 }}>
        ...
        {campanhaFilter && (
          <div style={{ marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <span style={{ fontSize: '12px', color: dark ? '#71717a' : '#6b7280' }}>Campanha:</span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '3px 10px', borderRadius: '99px', background: dark ? 'rgba(37,99,235,0.15)' : '#eff6ff', border: `1px solid ${dark ? 'rgba(37,99,235,0.3)' : '#bfdbfe'}`, fontSize: '12px', color: dark ? '#60a5fa' : '#2563eb', fontWeight: 500 }}>
              {campanhaFilter}
              <button onClick={() => setCampanhaFilter('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', display: 'flex', padding: 0 }}><X style={{ width: '11px', height: '11px' }} /></button>
            </span>
          </div>
        )}
                        ...
                        <tr key={lead.id} className={`${sel ? (dark ? 'bg-blue-950/30' : 'bg-blue-50/60') : idx % 2 === 0 ? '' : (dark ? 'bg-[#0f0f11]' : 'bg-gray-50/50')} ${hov} transition-colors cursor-pointer border-b ${divider} last:border-0`} onClick={() => setViewingLead(lead)}>
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-2">
                              ...
                              {obs && obs.trim() && (<div style={{ width: '16px', height: '16px', borderRadius: '50%', background: dark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={dark ? '#9ca3af' : '#6b7280'} strokeWidth="2.2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg></div>)}
                            </div>
                          </td>
                          <td className={`px-4 py-4 ${muted}`}>{lead.whatsapp || '—'}</td>
                          <td className={`px-4 py-4 ${muted}`}>{lead.cidade ? normalizeCity(lead.cidade) : '—'}</td>
                          <td className="px-4 py-4"><span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${badge.bg} ${badge.text}`}><span className={`w-1.5 h-1.5 rounded-full ${badge.dot}`} />{STATUS_LABELS[s]}</span></td>
