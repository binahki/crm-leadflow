...
// Retorna "YYYY-MM-DD" no fuso America/Sao_Paulo — comparável como string
...
function filterByPeriod(leads: Lead[], period: string, customFrom?: string, customTo?: string): Lead[] {
  ...
  switch (period) {
    ...
    case 'custom':    {
      ...
      return leads.filter(l => ok(l, customFrom, customTo));
    }
  ...
  const filtered = useMemo(() => {
    ...
    if (campanhaFiltro.trim()) {
      ...
      r = r.filter(l => {
        ...
        return camp.includes(q);
      });
    }
    // Busca manual (nome, whatsapp, cidade, campanha)
    if (search.trim() && !campanhaFiltro.trim()) {
      const q = search.toLowerCase();
      r = r.filter(l => {
        const la = l as any;
        const camp = (la.utm_campaign || '').toLowerCase().split('|')[0].trim();
        return l.nome?.toLowerCase().includes(q) || l.whatsapp?.includes(search) || l.cidade?.toLowerCase().includes(q) || camp.includes(q);
      });
    }
    return r;
